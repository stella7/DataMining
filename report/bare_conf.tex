...
The downloaded original files were too large to effectively load in R. We find a python script which can make a unique http request and retrieve data sample randomly. As our classification and clustering are based on the genre of the movie, there are many attributes are useless, such as seriesID, season, Episode, and etc. We choose to drop these columns during data cleaning process. There are 29 genres in total in our dataset. After processing, the movie dataframe contains 11142 rows of records and 31 attributes for each record.  
